...
train_itr = wavfile_caching_mel_tbptt_iterator(wavfiles, jsonfiles, batch_size, seq_len, symbol_processing="both", stop_index=.95, shuffle=True, random_state=train_random_state)
valid_itr = wavfile_caching_mel_tbptt_iterator(wavfiles, jsonfiles, batch_size, seq_len, symbol_processing="both", start_index=.95, shuffle=True, random_state=valid_random_state)

"""
for i in range(10000):
    print(i)
    mels, mel_mask, text, text_mask, text2, text2_mask, reset = train_itr.next_masked_batch()
"""
...
char_vocabulary_size = train_itr.vocabulary_sizes[0]
phone_vocabulary_size = train_itr.vocabulary_sizes[1]
...
def create_graph():
    ...
    with graph.as_default():
        ...
        text_char_e, t_c_emb = Embedding(text, char_vocabulary_size, emb_dim, random_state=random_state,
                                         name="text_char_emb")
        text_phone_e, t_p_emb = Embedding(text2, phone_vocabulary_size, emb_dim, random_state=random_state,
                                          name="text_phone_emb")
        conv_text = SequenceConv1dStack([text_char_e + text_phone_e], [emb_dim], n_filts, bn_flag,
                                        n_stacks=n_stacks,
