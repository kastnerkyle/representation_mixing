...
train_itr = wavfile_caching_mel_tbptt_iterator(wavfiles, jsonfiles, batch_size, seq_len, symbol_processing="blended_pref", stop_index=.95, shuffle=True, random_state=train_random_state)
valid_itr = wavfile_caching_mel_tbptt_iterator(wavfiles, jsonfiles, batch_size, seq_len, symbol_processing="blended_pref", start_index=.95, shuffle=True, random_state=valid_random_state)
...
def create_graph():
    ...
    with graph.as_default():
        ...
        mask_e, m_emb = Embedding(mask, 2, emb_dim, random_state=random_state,
                                  name="mask_emb")
        conv_text = SequenceConv1dStack([text_e + mask_e], [emb_dim], n_filts, bn_flag,
                                        n_stacks=n_stacks,
